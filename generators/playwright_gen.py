...
class PlaywrightGen:
    ...
    def generate_tests(self, project_name, project_api: dict[str, dict[str, list[Endpoint]]], output_path: str = "./output"):
        ...
        self._create_suite_files(project_api, base_path)
    ...
    def create_payload_builder_file(self, project_api: dict[str, dict[str, list[Endpoint]]], base_path: Path):
        for api_name, suites in project_api.items():
            api_name = re.sub(r'[-\s\.]', "", api_name)
            buiders_path = base_path / "src" / "builders" / api_name
            buiders_path.mkdir(parents=True, exist_ok=True)

            for suite, endpoints in suites.items():
                file_path = buiders_path / f"{suite}PayloadBuilders.ts"

                with file_path.open("w", encoding="utf-8") as f:
                    for endpoint in endpoints:
                        build_name = self.format_payload_build_name(endpoint.method, endpoint.path)

                        f.write(f"export function {build_name}() {{\n")
                        f.write("    return {\n")
                        f.write("        // Define payload\n")
                        f.write("    };\n")
                        f.write("}\n\n")
    ...
    def format_payload_build_name(self, method: str, path: str) -> str:
        ...
        return f"build{method.upper()}{name}Payload"
    
    def _create_test_files(self, project_api: dict[str, dict[str, list[Endpoint]]], base_path: Path):
        for api_name, suites in project_api.items():
            api_name = re.sub(r'[-\s\.]', "", api_name)
            ...
            for suite, endpoints in suites.items():
                file_path = tests_path / f"{suite}.spec.ts"

                with file_path.open("w", encoding="utf-8") as f:
                    f.write("import { test, expect } from '@playwright/test';\n\n")
                    f.write(f"test.describe('{suite}', () => {{\n\n")
    ...
    def _create_suite_files(self, project_api: dict[str, dict[str, list[Endpoint]]], base_path: Path):
        self._create_test_files(project_api, base_path)
        self.create_payload_builder_file(project_api, base_path)
